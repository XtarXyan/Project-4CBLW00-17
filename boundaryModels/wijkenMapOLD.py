...
# PLEASE READ:
# As the name implies, this is the old version of the wijken-mapping script,
# which was used to check the current separation of neighbourhoods in Eindhoven,
# and check for their fitting of the 15-minute city classification, alongside 
# the data extracted from the definition refinement script.
# 
# This script, though extremely similar was replaced due to certain inefficiencies:
# most notably the difficulty of expanding neighbourhoods and combining them,
# and the measuring of building density and service diversity. To salvage, I 
# created a copy of what worked into the FIXED version, and ran it with some modifications,
# which are listed at the beginning of that version.
# Most of the comments (where appropriate) stay the same.


# Enable caching for OSM queries
ox.settings.use_cache = True
ox.settings.timeout = 600  # Set timeout to 300 seconds
# This is an alternative Overpass API endpoint that is more reliable than the default one 
# (in my experience, at least a few dozen seconds faster)
...
tags = {
    ...
    'amenity': [
        ...
        'place_of_worship', 
        # Place of worship can be considered "key" ammenity for some, and so needs including.
    ],
    'leisure': [
...
# TAGS FOR DEBUGGING - commenting out since can easily be used for checking if visuals work.
# tags = {
#     'office': ['yes'],
#     'shop': ['yes'],
#     'amenity': ['yes'],
...
# The final property bounds are based on the definition refinement script values 
# (though somewhat fine-tuned, since the simulation is not perfectly accurate.)
# The numbers below
