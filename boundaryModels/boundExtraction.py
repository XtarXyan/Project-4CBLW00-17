...
# An hour, just to be safe (never took longer than that.)
# See the "wijkenMap" script to see why we're using this specific API instance.
...
#### "Sustainable urbanism: towards a framework for quality and optimal density?" by S. Lehmann, 
#### from which we can outline the 0.6-0.8 range for most transport-oriented "walkable cities."
#### The service diversity score is a bit more subjective, but I used the CityAccessMap criteria
#### both here and in the final classification script (hubPlacementModel)

### If you wish to run this yourself, please consider the fact that this may take up to 40 minutes
### to execute, and so I recommend setting the timeout value to something exceeding that 
...
# Update: I added, and then promptly removed 3 of the  neighbourhoods for two reasons:

# 1) The query time for the two Southeast Asian regions used (especially one of the sub-regions of 
# Shinjuku in Tokyo) was incredibly long, and offset the results by a significant margin - it was
# foolish to even consider something of that size, and not consider the incredibly high population density.

# 2) San Diego, though ranking high in walkability rankings, derailed the building density modifications completely,
# since the building density is so varied; I'll stick to just querying a single Portland neighbourhood,
# since it's one of the best in terms of walkability rating.

best_neighborhoods = [
    'Amsterdam, Netherlands',  # Broader query for similar pop. density to Centrum regions
    'Friedrichshain, Berlin, Germany', # Similar in size/density to Stratum
