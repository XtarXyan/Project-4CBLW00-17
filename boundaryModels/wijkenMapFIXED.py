...
# Documentation now recommends using union_all instead of unary_union, but
# but most StackOverflow examples still use unary_union, so it's used at times where I
# was unsure of how to combine geometries.
from shapely.ops import unary_union

# Threading pool used to speed up the isochrone generation process.
# I'd like to thank the 2INC0 course for teaching me how to use this. 
from concurrent.futures import ThreadPoolExecutor


# Enable caching for OSM queries: we need this so that pulling data does not lead to a timeout.
# Also, switched to private.coffee - apparently .kumi.systems is deprecated! 
# The .coffee API has no rate limit in place, and uses the largest server config, so should help ou
# even more in terms of runtime.
ox.settings.use_cache = True
ox.settings.timeout = 600  # Set timeout to 600 seconds
...
## really necessary anymore (since the original model, alongside the analysis of CityAccessMap,
##  already shows the walkability of Eindhoven's districts/wijken.), so instead this will use 
##  Fieke's definitions of a 15-minute city, as based on the bounds extracted in definitionRefinement.py
## , and will try to separate Eindhoven into 15-minute zones that way.
...
### IMPORTANT NOTE:
# This is unfortunate to mention, but building density had to be scrapped from the simulation:
# querying OSMNx for building density made the script run for an unberably long time,
# and the results were very odd, since the density of Eindhoven is very varied - shifting Shannon
# diversity by ~0.05 eliminated a very large part of the city, even though checking with the
# geopackage provided by CBS shows that the building density is actually relatively "optimized"
# in the removed regions, indicating that even though the calculation was okay, the OSMNx data must
# have been incomplete. Service diversity-checking was also moved to a later part of the simulation,
# so that we may have a rubric for "improving" the neighbourhoods, meaning that this file is mainly
# concerned with population density and proper 15-minute walkability, the basis for our identifying
# of 15-minute-capable neighbourhoods.
...
## Building density: 0.6 - 0.8 built zone per total area // REMOVED
...
# Made this dependent on input for ease of use, since I ran this script dozens if not hundreds of times.
...
# Even though this is for debugging, this is useful to see if the mapping is accurate to the code;
# it was a long road to learn Folium, and I want to make sure that the results are correct.
