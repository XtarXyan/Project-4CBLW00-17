...
from shapely.geometry import MultiPolygon, Polygon # these are required for PROPER neighborhood allignment.
from geopandas import GeoSeries # not part of standard geopandas library?
...
# COMMENT FOR COMMENT: You still need to filter for eindhoven neighborhoods, otherwise you
# end up with municipalities surrounding Eindhoven alongside Eindhoven itself.

print('Loading boundaries.')

admin_tags = {"boundary": "administrative", "admin_level": "10"}

# This replacement instead of using Polygon boundaries should (hopefully) let the program
# draw lines alongside the administrative tags I described above (level 10, admin),
# and only then use Polygon boundaries to find those regions! I think.
neighborhoods_Raw = ox.features_from_place(place, tags = admin_tags)
crs = crs = G_walk.graph['crs']

# Filter for Eindhoven neighborhoods. This was the missing step.
eindhoven_names = ['Woensel', 'Strijp', 'Centrum', 'Gestel', 'Stratum', 'Tongelre', 'Acht']
neighborhoods = neighborhoods_Raw[
    neighborhoods_Raw['name'].isin(eindhoven_names)
].copy()

# Previous filter-less part:
...
def analyze_access(graph, center_node, radius_m, tags):
    ...
    isochrone_poly = GeoSeries(nodes.geometry).union_all().convex_hull #should fix the "attribute is deprecated" errors.
...
valid_results = [r for r in results if isinstance(r.get('geometry'), (Polygon, MultiPolygon))]

print("Top neighborhoods by 15mC classification:")

# --> Results:

results_sorted = sorted(valid_results, key=lambda x: -x['improvement'])
...
gdf_results = gpd.GeoDataFrame(valid_results, geometry='geometry', crs=neighborhoods.crs)


## New Folium map.
# Center map on Eindhoven (last one was global for some reason?)
map_center = neighborhoods.unary_union.centroid
...
cmap.add_to(m)
m.save('Eindhoven15mCFolium')
print("HTMLMap saved to directory.")